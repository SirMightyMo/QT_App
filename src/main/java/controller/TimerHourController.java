...
public class TimerHourController implements ActionListener, DocumentListener {
	...
	public void actionStartTimer() {
		...
		if (this.hourEntry == null) {
			...
			createHourEntry();
		} else if (this.hourEntry.getPauseStart() != null && this.hourEntry.getPauseEnd() == null && !this.timerModel.isTimerRunning()) {
	...
	// Calculates duration from input field for setting view (not connected to hour entry)
	public void actionCalculateDurationView() {
		if (!timerModel.isTimerRunning()) {
			...
			if ((pause.matches("^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$") || 
					pause.matches("^\\d+[,||.]\\d{2}$") || 
					pause.matches("")) && 
					start.matches("^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$") &&
				...
				// if end time is before starttime, next day has started. Therefore the productive minutes are calculated
				if (LocalTime.parse(end, DateTimeFormatter.ofPattern("H:mm")).compareTo(LocalTime.parse(start, DateTimeFormatter.ofPattern("H:mm"))) < 0) {
				...
				// else (start lies before end) entries are made for the same day
					...
					long durationSeconds = Duration.between(LocalTime.parse(start, DateTimeFormatter.ofPattern("H:mm")), LocalTime.parse(end, DateTimeFormatter.ofPattern("H:mm"))).getSeconds();
	 ...
	 * actionSaveTimer()
	 * 
	 * Calls method 'actionStopTimer()' which stops the timer and writes all entries based on current time.
	 * The 'actionStopTimer()' method checks first, if the timer is running. When the timer is stopped, all
	 * editable time fields will be automatically filled with the needed information.
	 * 
	 * This means:
	 * If the timer was running, there was already created an hour entry and the timer will be stopped.
	 * If the timer was not running, fields will be empty if not edited manually.
	 * 
	 * This method 'actionSaveTimer()' will create an hour entry, if none was found (this.hourentry == null).
	 * 
	 * It then will not only read the editable form fields, but it will also overwrite the information 
	 * in the hour entry before reading it again for inserting it into the database. 
	 * This way the used instance of HourEntry will temporarily contain the same information written to
	 * the database, in case it is needed elsewhere later.
	 *  
	 */
	public void actionSaveTimer() {
		// stop timer if it is running and set timefields (start, stop, pause) with information
		// when timer is stopped, editable time field are automatically filled with needed information
		// 
		if (this.timerModel.isTimerRunning()) {
			actionStopTimer();		
		}
		...
		// if timer was not used (not started), hour entry was not created and needs to be done here
		if (this.hourEntry == null) {
		...
		// read PauseDuration from textfield; needs to be done before setting startTime & endTime, because sessionTime is calculated considering pauseduration
        ...
        if (pauseMinutesString.matches("^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$") || pauseMinutesString.matches("^\\d+[,||.]\\d{2}$")) {
        ...
        if (startTimeField.equals("") || !startTimeField.matches("^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$")) {
        	...
        	// create LocalDateTime for startTime, write to hourEntry, read from hourEntry for database
        	LocalDateTime startDateTimeField = LocalDateTime.of(dateEntryLocalDate, LocalTime.parse(startTimeField, DateTimeFormatter.ofPattern("H:mm")));
    	...
    	// read end time from textfield			
         ...
         if (endTimeField.equals("") || !endTimeField.matches("^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$")) {
         	...
         	// create LocalDateTime for endTime, write to hourEntry, read from hourEntry for database
         	long plusDays = 0;
        	if (endTimeField.compareTo(startTimeField) < 0) { // if endtime is smaller than starttime, it means that another day has started and a day needs to be added
         		plusDays = 1;
         	}
        	LocalDateTime endDateTimeField = LocalDateTime.of(dateEntryLocalDate.plusDays(plusDays), LocalTime.parse(endTimeField, DateTimeFormatter.ofPattern("H:mm")));
